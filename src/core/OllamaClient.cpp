...
size_t WriteCallback(void *contents, size_t size, size_t nmemb,
  ...
  output->append(reinterpret_cast<char *>(contents), total);

  return total;
}

OllamaClient::OllamaClient(const std::string &model) : model(model) {}
...
std::string OllamaClient::chat(const std::vector<Message> &history) {
  ...
  for (int i = start; i < history.size(); i++) {
    ...
    request["messages"].push_back({{"role", history[i].role},
